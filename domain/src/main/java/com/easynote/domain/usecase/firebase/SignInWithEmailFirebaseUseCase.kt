...
class SignInWithEmailFirebaseUseCase(private val authRepository: AuthenticationRepository) {

    fun execute(credential: UserCredential): Flow<NetworkResult<FirebaseUser>> = callbackFlow {
        trySend(NetworkResult.Loading())
        authRepository.signInWithEmail(credential).addOnCompleteListener { task ->
